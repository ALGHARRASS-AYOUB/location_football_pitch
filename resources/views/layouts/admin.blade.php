...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        ...
        <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
            ...
            <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-slate-900 bg-white md:w-64 dark:text-gray-200 dark:bg-gray-400" x-data="{ open: false }">
                ...
                <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
                    ...
                    <a href="{{ route('admin.index') }}" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark:text-white focus:outline-none focus:shadow-outline">Admin Panel</a>
                    @endif
                    @if (Auth::user()->role=='manager')
                    <a href="{{ route('admin.index') }}" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark:text-white focus:outline-none focus:shadow-outline">Manager Panel</a>
                    @endif
                ...
                <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
                    ...
                    <x-admin-nav-link :href="route('admin.pitches.index')" :active="request()->routeIs('admin.pitches.index')">
                        {{ __('Pitches') }}
                    </x-admin-nav-link>

                    <x-admin-nav-link :href="route('admin.periods.index')" :active="request()->routeIs('admin.periods.index')">
                        {{ __('Periods') }}
                    </x-admin-nav-link>
                    ...
                    <div @click.away="open = false" class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg">
                            <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-700">
                                ...
                                <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="#">My Account</a>
                                <!-- Authentication -->
