<x-admin-layout>
    ...
    <div class="container bg-gray-100 m-3 p-8 rounded-lg">
        <form enctype="multipart/form-data" action="{{ route('admin.menus.update',$menu->id) }}" method="POST">
            ...
            {{-- <div class="grid md:grid-cols-2 md:gap-6"> --}}
                ...
                <input type="text" name="name" id="name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{ $menu->name }}" required="">
                <label for="name" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ $menu->name }}</label>
            {{-- </div> --}}
        </div>

                    {{-- <div class="grid md:grid-cols-2 md:gap-6"> --}}
                        <div class="relative z-0 mb-6 w-full group">
                            <input type="text" name="price" id="price" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{ $menu->price }} " required="">
                            <label for="price" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ $menu->price }} DH</label>
                        {{-- </div> --}}
                    </div>

                    <span>old image</span>
                    <div class="mb-6 w-16 h-16 mx-2 my-4">
                        <img src="{{ Storage::url($menu->image) }}" class="h-16 w-16 rounded-2xl" alt="">
                    </div>

            <div class="grid md:grid-cols-2 md:gap-6 mt-8 mb-4">
                <div class="relative z-0 mb-6 w-full group">
                <input name="image" id="image" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"  id="image" type="file" value="{{ $menu->image }}">
  ...
  {{-- <div class="bg-yellow-400">  {{ $menu->categories  }}</div>
    <div class=" bg-green-300"> --}}
        @php
         function menuCategoriesIDs($categories){
            $menu_cat_ids=[];
            if($categories=='')
                return $menu_cat_ids;
        for ( $i=0;$i<sizeof($categories);$i++){
              $menu_cat_ids[$i]=$categories[$i]['id'];
        }
        return $menu_cat_ids;
        }

         function is_match($item,$list){
                ...
                <ul class="overflow-y-auto px-3 pb-3 h-48 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownSearchButton">
                    ...
                    <li>
                        ...
                        <div class="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                            <input name="categories[]" id="checkbox-item-{{ $category->id }}" type="checkbox" value="{{ $category->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500"  @checked(is_match($category->id,menuCategoriesIDs($categoryOfThisMenu)))>
